...
local e,o=dget(n+30),dget(n+20)local l=e==0and o==0if self.time_mode=="countdown"do if(l or self.time_remaining>e*6000+o)dset(n+20,self.time_remaining)dset(n+30,0)
...
return v(n*6000+e)end function n:update_score_line_clear(e,o,n)self.lines_cleared+=n self.level=flr(self.lines_cleared/10)+1self.drop_interval=max(5,self.drop_interval_max-(self.level*2-2))self:add_score(e[n]*self.level)if(self.challenge.name=="rush"and self.time_remaining and n>0)self.time_remaining+=o[n]
...
return v(n)end function v(n)local n,e=n\60,n%60return(n<10and"0"..n or tostring(n))..":"..(e<10and"0"..e or tostring(e))end function y(n,e,o,l,d)local t,o=#n*4,o-e local e=e+(o-t)/2?n,e,l,d
...
fillp(d[n]+.5)rectfill(0,0,127,127,0)fillp()end function _init()cartdata"yapt"poke(24366,1)k={[0]=0,0,1,1,2,1,5,6,2,4,9,3,1,1,2,5}c="menu"m=nil p=e:new()f=o:new()i=false end function _update60()if c=="playing"do m:update_world()I()elseif c=="menu"do p:update_menu()end f:update()end function _draw()cls(0)if c=="playing"do m:draw_world()b()elseif c=="menu"do p:draw_menu()end if(f.active)f:draw()
end function O(e)pal()if(e=="playing")m=n:new(p:current_challenge())
c=e end
__gfx__
